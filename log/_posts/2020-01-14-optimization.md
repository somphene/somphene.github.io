...
* Define a $$\textbf{Ball of radius} r>0$$ in $$\mathbb{R}^n$$ as: 
$$B(x,r)= \{ y \in \mathbb{R}^n \, | \,\,  \lVert y-x \rVert  < r \} $$
* $$f \colon \mathbb{R}^n \to \mathbb{R} $$ is said to be $$\textbf{continuous at a point}$$ $$x \in \mathbb{R}^n$$ if $$\forall \epsilon >0, \quad \exists \delta > 0, \quad$$ such that $$ \lvert f(y)-f(x) \rvert < \epsilon, \quad \forall y \in B(x,\delta) $$
