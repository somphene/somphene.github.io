...
* Intuition behind $$L_1$$ being better norm than $$L_p$$ for optimal solution. Starting with near zero $$L_1$$ norm, draw balls of increasing norm and see that the optimal solution is sparse (min is single point on the axis where the contraint equation line becomes tangent to the $$L_1$$ norm ball). Whereas in the case of $$L_2$$ norm, the optimal point is off the axis and hence not sparse. Saumya asked whether this means the optimal solution is non-zero only along one axis.
